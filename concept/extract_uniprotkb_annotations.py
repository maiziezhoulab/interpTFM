...
def expand_features(
    ...
    for col in binary_cols:
        ...
        if not non_na_values.empty:
            col_name = non_na_values.iloc[0].split(" ")[0]
        else:
            print(f"Warning: No non-NA values in column '{col}'")
            col_name = None
...
def convert_shard_to_amino_acid_features(
    ...
    # aa_concepts is the 0/1 matrix of shape len(aa) X len(functionality)
    # aa_concepts_columns is the header (columns)
    # aa_metadata is the row idx (aas from the proteins) of the concepts

    # gene_concepts: celltype+gene exp -> pathways /
    # cell_concepts 
...
if __name__ == "__main__":
    # tapify(main)
    main(
        input_uniprot_path = '/maiziezhou_lab2/yunfei/Projects/FM_temp/InterPLM/interplm/uniprotkb/proteins.tsv.gz',
        output_dir= Path("/maiziezhou_lab2/yunfei/Projects/FM_temp/InterPLM/interplm/uniprotkb/annotations"),
        n_shards= 8,
        min_required_instances= 10,
        min_protein_length = 1022,
        overwrite = False,
    )
