...
def calculate_feature_statistics(
    ...
    for shard in range(n_shards):
        ...
        shard_path = esm_embds_dir / f"shard_{shard}" / "activations.pt"
...
def normalize_sae_features(
    sae_dir: Path, embds_dir: Path, n_shards: Optional[int] = 1
) -> None:
    """
    ...
    Args:
        ...
        embds_dir: Directory containing ESM embeddings
        n_shards: Number of data shards to process
    """
    # Setup paths
    ...
    max_per_feat = calculate_feature_statistics(
        sae=sae, esm_embds_dir=embds_dir, n_shards=n_shards
    )
